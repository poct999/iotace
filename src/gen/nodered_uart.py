...
def _build_html(i) -> str:
    result = tmp.render_tpl('nodered_uart', 'html.tpl',
...
def _build_js(i) -> str:
    result = tmp.render_tpl('nodered_uart', 'js.tpl',
                            {"function": bd.functions[i], "module_name": bd.config["program_name"]})
...
def _build_package() -> str:
    ...
    result = tmp.render_tpl('nodered_uart', 'package.tpl',
...
def _build_dep():
    global module_dir
    module_dir = bd.config["build_path"] + '/nodered_uart_%s' % bd.config["program_name"]
